...
document.addEventListener('DOMContentLoaded', function() {
  ...
  const prevBtn2 = document.querySelector('.modal-prev');
  const nextBtn2 = document.querySelector('.modal-next');
  ...
  if (modal && modalImg && thumbnails.length) {
    ...
    document.addEventListener('keydown', function(e) {
      if (modal.style.display === 'block') {
        if (e.key === 'Escape') {
          modal.style.display = 'none';
        } else if (e.key === 'ArrowLeft' && prevBtn2) {
          prevBtn2.click();
        } else if (e.key === 'ArrowRight' && nextBtn2) {
          nextBtn2.click();
        }
      }
    });
